...
from core.utils import duration_to_hours, duration_string_short_hm
...
def sleep_totals(instances):
    ...
    trace = go.Bar(
        name=_("Total sleep"),
        x=list(totals.keys()),
        y=[duration_to_hours(td) for td in totals.values()],
        hoverinfo="text",
        textposition="outside",
        text=[duration_string_short_hm(td) for td in totals.values()],
    )
